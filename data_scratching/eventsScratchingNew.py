...
while (page_url):
	...
	for i in range(len(event_list)):
		...
		url_xpath = '//*[@id="block-system-main"]/div/div/div[1]/div/ul/li['+str(i+1)+']/div/div[1]/div[1]/div/div/h3/a/@href'
		...
		try:
			...
			event_image = 'null'
		event_url = page.xpath(url_xpath)[0]
		...
		event_element['url'] = str(page_url+event_url)
...
events_json = open('data11111111.json').read()
events_data = json.loads(events_json)
location_json = open('eventsLocations.json').read()
...
for item in events_data:
	...
	try:
		...
		print "No location found for", item['title']

print count
with open('eventsData.json', 'w') as f:
